...
## 2026-05-27 - Public Exposure of Sensitive Inventory Metrics
**Vulnerability:** The `lowStockProductsCount` query was publicly accessible without authentication or role checks, allowing competitors to probe inventory levels. It also lacked input clamping on the `threshold` parameter, allowing for potentially exhaustive data probing.
**Learning:** Sensitive business intelligence can be leaked through seemingly harmless aggregation queries. Authorization must be applied to queries that expose non-public business metrics, not just mutations or direct PII access.
**Prevention:** Audit all "stats" or "count" queries to ensure they follow the principle of least privilege. Implement strict bounds on parameters that influence query logic to prevent data probing.
